(() => {
  ...
  const servo = () => {
    ...
    Blockly.JavaScript.forBlock["uno_servo"] = function (block) {
    ...
    // Geração de código Python para o bloco
    Blockly.JavaScript.forBlock["uno_servo360"] = function (block) {
  ...
  const categoriaServo = {
    ...
    colour: COR_BLOCOS,
    contents: [
      { kind: "block", type: "uno_servo" },
      { kind: "block", type: "uno_servo360" },
    ],
  };

  // Registra blocos
  window.unoRegistry = window.unoRegistry || [];
  window.unoRegistry.push({ init: servo, category: categoriaServo });
})();
